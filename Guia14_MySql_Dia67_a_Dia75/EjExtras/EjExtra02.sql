...
SELECT puesto, nombre, apellido1, apellido2, email FROM empleado WHERE codigo_empleado = codigo_jefe;
...
-- 8. Devuelve un listado con el código de cliente de aquellos clientes que realizaron algún pago en 2008. Elimina los códigos de cliente repetidos.
...
-- 13. Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordena el resultado de mayor a menor.
SELECT * FROM pago WHERE YEAR(fecha_pago) = 2008 AND forma_pago = 'Paypal' ORDER BY total DESC;

-- 14. Devuelve un listado con todas las formas de pago que aparecen en la tabla pago. No deben aparecer formas de pago repetidas.
SELECT DISTINCT forma_pago FROM pago;

-- 15. Devuelve un listado con todos los productos que pertenecen a la gama "Ornamentales" y que tienen más de 100 unidades en stock. El listado debe estar ordenado por su precio de venta, mostrando primero los de mayor precio.
SELECT * FROM producto WHERE gama = 'Ornamentales' AND cantidad_en_stock > 100 ORDER BY precio_venta DESC;

-- 16. Devuelve un listado con todos los clientes que son de la ciudad de Madrid y cuyo representante de ventas tiene el código de empleado 11 o 30.
SELECT * FROM cliente WHERE ciudad = 'Madrid' AND codigo_empleado_rep_ventas IN (11, 30);

-- CONSULTA MULTITABLA
-- Punto 1: Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
SELECT c.nombre_cliente, e.nombre, e.apellido1
FROM cliente c
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado;

-- Punto 2: Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT c.nombre_cliente, e.nombre, e.apellido1
FROM cliente c
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
INNER JOIN pago p ON c.codigo_cliente = p.codigo_cliente;

-- Punto 3: Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT c.nombre_cliente, e.nombre, e.apellido1
FROM cliente c
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
-- Punto 4: Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
...
-- Punto 5: Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT c.nombre_cliente, e.nombre, e.apellido1, o.ciudad
FROM cliente c
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
LEFT JOIN pago p ON c.codigo_cliente = p.codigo_cliente
...
-- Punto 6: Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
...
-- Punto 7: Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT c.nombre_cliente, e.nombre, e.apellido1, o.ciudad
FROM cliente c
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
INNER JOIN oficina o ON e.codigo_oficina = o.codigo_oficina;

-- Punto 8: Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
...
-- Punto 9: Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
SELECT c.nombre_cliente
FROM cliente c
INNER JOIN pedido p ON c.codigo_cliente = p.codigo_cliente
WHERE p.fecha_entrega > p.fecha_esperada;

-- Punto 10: Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
...
WHERE pa.codigo_cliente IS NULL AND pe.codigo_pedido IS NULL;
...
SELECT MAX(precio_venta) AS precio_maximo, MIN(precio_venta) AS precio_minimo FROM producto;
...
SELECT ciudad, COUNT(*) AS total_clientes FROM cliente WHERE ciudad LIKE 'M%' GROUP BY ciudad;
...
SELECT c.nombre_cliente, c.nombre_contacto, c.apellido_contacto, MIN(p.fecha_pago) AS primer_pago, MAX(p.fecha_pago) AS ultimo_pago
FROM cliente c
LEFT JOIN pago p ON c.codigo_cliente = p.codigo_cliente
GROUP BY c.nombre_cliente, c.nombre_contacto, c.apellido_contacto;

-- 12. Calcula el número de productos diferentes que hay en cada uno de los pedidos.
SELECT codigo_pedido, COUNT(DISTINCT codigo_producto) AS total_productos_diferentes
FROM detalle_pedido
...
SELECT SUM(precio_venta * cantidad) AS base_imponible, (SUM(precio_venta * cantidad) * 0.21) AS iva, SUM(precio_venta * cantidad) + (SUM(precio_venta * cantidad) * 0.21) AS total_facturado
FROM detalle_pedido dp
JOIN producto p ON dp.codigo_producto = p.codigo_producto;

-- 16. La misma información que en la pregunta anterior, pero agrupada por código de producto.
SELECT dp.codigo_producto, SUM(p.precio_venta * dp.cantidad) AS base_imponible, (SUM(p.precio_venta * dp.cantidad) * 0.21) AS iva, SUM(p.precio_venta * dp.cantidad) + (SUM(p.precio_venta * dp.cantidad) * 0.21) AS total_facturado
FROM detalle_pedido dp
JOIN producto p ON dp.codigo_producto = p.codigo_producto
...
SELECT p.nombre, SUM(dp.cantidad) AS unidades_vendidas, SUM(p.precio_venta * dp.cantidad) AS total_facturado, SUM((p.precio_venta * dp.cantidad) * 0.21) AS total_facturado_con_iva
FROM detalle_pedido dp
JOIN producto p ON dp.codigo_producto = p.codigo_producto
GROUP BY p.nombre
...
SELECT nombre_cliente
FROM cliente
WHERE limite_credito > (SELECT COALESCE(SUM(total), 0) FROM pago WHERE codigo_cliente = cliente.codigo_cliente);
...
FROM empleado
WHERE codigo_empleado IN (SELECT codigo_empleado FROM empleado WHERE CONCAT(nombre, ' ', apellido1) = 'Alberto Soria');
...
SELECT nombre
FROM producto
WHERE cantidad_en_stock <= ANY (SELECT cantidad_en_stock FROM producto);
...
SELECT *
FROM producto p
WHERE EXISTS (SELECT * FROM detalle_pedido WHERE codigo_producto = p.codigo_producto);